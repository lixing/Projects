/*
 * File: graphimpl.cpp
 * Last modified on Tue Feb 22 11:23:39 2011 by eroberts
 * -----------------------------------------------------
...
void Graph<NodeType,ArcType>::copyInternalData(const Graph & other) {
	nodes = Set<NodeType *>(NodeCompare<NodeType>);
	Set<NodeType *>nodeSet = other.nodes;
	foreach (NodeType *oldNode in nodeSet) {
		NodeType *newNode = new NodeType();
		*newNode = *oldNode;
		newNode->arcs.clear();
		addNode(newNode);
	}
	arcs = Set<ArcType *>(ArcCompare<NodeType,ArcType>);
	Set<ArcType *> arcSet = other.arcs;
	foreach (ArcType *oldArc in arcSet) {
